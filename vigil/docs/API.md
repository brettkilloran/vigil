...
**Tool names:** **`tools/list`** advertises only **`heartgarden_*`** (e.g. **`heartgarden_search`**, **`heartgarden_patch_item`**, **`heartgarden_create_item`**, **`heartgarden_create_link`**). **`tools/call`** still accepts legacy **`vigil_*`** names and maps them to **`heartgarden_*`** via **`canonicalHeartgardenMcpToolName`** (see unit tests in **`heartgarden-mcp-server.test.ts`**). Write tools **`heartgarden_create_item`** and **`heartgarden_create_link`** validate **`write_key`** against **`HEARTGARDEN_MCP_WRITE_KEY`** in the MCP process (same as **`heartgarden_patch_item`**); **`write_key`** may be omitted when the MCP server has **`HEARTGARDEN_MCP_WRITE_KEY`** set. **`heartgarden_create_item`** calls **`POST /api/spaces/:id/items`**; **`heartgarden_create_link`** calls **`POST /api/item-links`** (optional **`relationship_type`** maps to **`linkType`**).
...
| POST | `/api/spaces/[spaceId]/items` | Create item in space. Body: **`itemType`**, **`x`**, **`y`**, optional **`width`**, **`height`**, **`title`**, **`contentText`**, **`contentJson`**, optional **`theme`** (`default` \| `code` \| `task`, used when synthesizing **`contentJson`**). If **`zIndex`** is omitted, the server assigns **`max(zIndex)+1`** in that space (starts at **101** when empty). If **`width`** / **`height`** are omitted, defaults depend on type: note **340×270**, checklist **340×188**, note with **`entityType`** **`character`** **340×453**, other types **280×200**. If **`contentJson`** is omitted for **`note`**, **`sticky`**, or **`checklist`**, the server synthesizes a minimal **`html`** or **`hgDoc`** body plus **`hgArch`** (small random rotation jitter). **`contentText`** is required when **`contentJson`** is omitted for **`note`** rows. |
