...
| **Viewport arrival (origin)** | Bootstrap, opening a folder, and related transitions land at **world origin** (0,0), zoom 1 — not half-window offsets or restoring stale localStorage pan; pan/zoom still persist while you work | [`AGENTS.md`](../AGENTS.md) (Canvas camera) | `defaultCamera()` in `canvas-types.ts`, `heartgarden-space-camera.ts`, `applyBootstrapData` / `enterSpace` / `recenterToOrigin` in `ArchitecturalCanvasApp.tsx` |
