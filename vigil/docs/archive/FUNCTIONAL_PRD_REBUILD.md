...
- Account-based auth, permissions, and **full** realtime co-editing (CRDT/OT / Google-Docs-style merge). **Soft presence** (no accounts) — optional **`canvas_presence`**, subtree peer list, remote cursors, and **follow** another tab’s viewport — is implemented in cloud mode; see **`../PLAYER_LAYER.md`**.
...
- **Current production note:** the shipped shell keeps the **canvas graph** in **`ArchitecturalCanvasApp`** (in-component state + undo/redo). A **Zustand** store may still exist for **panels / legacy paths** — verify in `src/stores/canvas-store.ts` and **`../CODEMAP.md`** before extending.
...
- `PATCH /api/spaces/:spaceId`
  - Update **name** / **parentSpaceId**; **`camera`** in body is accepted but **ignored** for persistence (see note above).
