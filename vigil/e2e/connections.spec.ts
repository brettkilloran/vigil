...
test.describe("connection pin anchoring", () => {
  ...
  test("anchors drawn string pins to source and target cards", async ({ page }) => {
    ...
    await expect.poll(async () => page.locator("svg[aria-hidden] circle").count()).toBeGreaterThanOrEqual(2);

    const attached = await page.evaluate(({ sourceNodeId, targetNodeId }) => {
      const sourceEl = document.querySelector<HTMLElement>(`[data-node-id="${sourceNodeId}"]`);
      const targetEl = document.querySelector<HTMLElement>(`[data-node-id="${targetNodeId}"]`);
      const svg = document.querySelector<SVGSVGElement>("svg[aria-hidden]");
      if (!sourceEl || !targetEl || !svg) return null;
