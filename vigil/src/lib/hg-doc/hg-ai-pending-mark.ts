...
export function expandHgAiPendingRangeBeforeEdit(
  ...
  for (let i = 0; i < merged.length - 1; i++) {
    ...
    if (gap > HG_AI_PENDING_SPLIT_GAP_MAX) continue;
    if (!areHgAiPendingRunsInSameTextblock(doc, left.to, right.from)) continue;
      ...
      new Plugin({
        ...
        appendTransaction(transactions, oldState, newState) {
          ...
          for (const tx of transactions) {
            ...
            const expanded = expandHgAiPendingRangeBeforeEdit(oldState, step.from, markType);
