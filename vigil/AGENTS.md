...
Tools include **`heartgarden_browse_spaces`**, **`heartgarden_space_summary`**, **`heartgarden_list_items`**, **`heartgarden_search`** (default `hybrid`), **`heartgarden_semantic_search`**, **`heartgarden_graph`**, **`heartgarden_get_item`** / **`heartgarden_get_entity`**, **`heartgarden_item_links`**, **`heartgarden_traverse_links`**, **`heartgarden_related_items`**, **`heartgarden_title_mentions`**, **`heartgarden_lore_query`**, **`heartgarden_index_item`**, **`heartgarden_reindex_space`**, **`heartgarden_patch_item`**, **`heartgarden_create_item`**, **`heartgarden_create_link`**. **`HEARTGARDEN_MCP_WRITE_KEY`** must match `write_key` on **`heartgarden_patch_item`**, **`heartgarden_reindex_space`**, **`heartgarden_create_item`**, and **`heartgarden_create_link`** (create tools also allow omitting `write_key` when this env is set on the MCP process). **`HEARTGARDEN_MCP_SERVICE_KEY`**: required on the server for **`/api/mcp`** (503 if unset); use the same value when calling tools so internal `fetch` calls pass the boot gate in production (**`Authorization: Bearer …`**). **Resources:** `lore://space/<uuid>`. Defaults: **`HEARTGARDEN_DEFAULT_SPACE_ID`**, **`HEARTGARDEN_APP_URL`**. The Next app must be reachable for HTTP tool calls. **Legacy:** clients may still invoke **`vigil_*`** tool names; the server maps them to **`heartgarden_*`**.
