...
import { parseBool, parseCsv, parseSearchFiltersFromUrl } from "@/src/lib/heartgarden-search-url-params";
...
function stableFiltersSnapshot(f: ReturnType<typeof parseSearchFiltersFromUrl>) {
  return {
    ...f,
...
describe("parseSearchFiltersFromUrl", () => {
  ...
  it('variant "suggest" matches legacy /api/search/suggest subset', () => {
    const u = new URL("http://localhost/api/search/suggest?spaceId=sp&types=folder&limit=5&hasLinks=false");
