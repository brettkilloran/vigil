...
function applyCharacterMarkerTiltVars(
  ...
  if (Math.abs(trimTop - trimBottom) < 10) {
    trimBottom = Math.min(
      trimTop + 10 + seededFloat(fieldSeed, "trim-uniformity", 0, 22),
      52
    );
  }
  ...
  el.style.setProperty(
    "--sk-v11-marker-trim-right-top",
    `${trimTop.toFixed(2)}px`
  );
  el.style.setProperty(
    "--sk-v11-marker-trim-right-bottom",
    `${trimBottom.toFixed(2)}px`
  );
...
function applyOrdoV7MarkerTiltVars(el: HTMLElement, fieldSeed: string): void {
  ...
  if (Math.abs(trimTop - trimBottom) < 6) {
    trimBottom = Math.min(
      trimTop + 6 + seededFloat(fieldSeed, "trim-uniformity", 0, 12),
      28
    );
  }
...
function seedCharShellV11(shell: HTMLElement): void {
  const headerMeta = shell.querySelector<HTMLElement>(
  ...
  for (const el of shell.querySelectorAll<HTMLElement>(
    ...
    if (
      el.matches?.('[class*="charSkNotesBody"]') ||
      el.matches?.('[class*="charSkHeaderMeta"]')
    ) {
      continue;
    }
    if (seededMarkerFieldElements.has(el)) {
      continue;
    }
...
function seedLocOrdoV7Root(root: HTMLElement): void {
  ...
  for (const el of root.querySelectorAll<HTMLElement>("[data-hg-lore-field]")) {
    if (el.closest("[data-hg-lore-location-notes]")) {
      continue;
    }
    if (seededMarkerFieldElements.has(el)) {
      continue;
