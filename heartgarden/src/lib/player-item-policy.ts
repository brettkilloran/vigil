...
const PLAYERS_ALLOWED_ITEM_TYPES = new Set([
  "note",
  "sticky",
  "checklist",
  "folder",
...
export function stripGmOnlyEntityMetaPatch(
  ...
  for (const [k, v] of Object.entries(patch)) {
    if (GM_ONLY_ENTITY_META_KEYS.has(k)) {
      continue;
    }
    out[k] = v;
  }
...
export function playersPatchBodyViolatesPolicy(body: {
  ...
  if (!playersMayPatchItemType(body.itemType)) {
    return true;
  }
  if (body.imageUrl !== undefined && body.imageUrl !== null) {
    return true;
  }
  if (body.imageMeta !== undefined && body.imageMeta !== null) {
    return true;
  }
