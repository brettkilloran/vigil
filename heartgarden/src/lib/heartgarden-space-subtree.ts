...
export async function fetchPlayerSubtreeSpacesFull(
  db: VigilDb,
  playerRootSpaceId: string
) {
  ...
  if (!allowedIds.has(playerRootSpaceId) || allowedIds.size === 0) {
    return [];
  }
  return await db
    .select()
    .from(spaces)
    .where(inArray(spaces.id, [...allowedIds]));
}
