 ...
 */
export function parseHybridRetrieveQueryParams(searchParams: URLSearchParams): Partial<HybridRetrieveOptions> {
  const out: Partial<HybridRetrieveOptions> = {};
  const fts = parseOptPositiveInt(searchParams.get("ftsLimit"));
  if (fts !== undefined) out.ftsLimit = clampInt(fts, 8, 200);
  const fe = parseOptPositiveInt(searchParams.get("fuzzyLimitEmpty"));
  if (fe !== undefined) out.fuzzyLimitWhenEmpty = clampInt(fe, 8, 100);
  const fs = parseOptPositiveInt(searchParams.get("fuzzyLimitSparse"));
  if (fs !== undefined) out.fuzzyLimitWhenSparse = clampInt(fs, 8, 100);
  const thr = parseOptPositiveInt(searchParams.get("ftsSparseThreshold"));
  if (thr !== undefined) out.ftsSparseThreshold = clampInt(thr, 2, 64);
  const vc = parseOptPositiveInt(searchParams.get("vectorChunkLimit"));
  if (vc !== undefined) out.vectorChunkLimit = clampInt(vc, 8, 200);
  const mc = parseOptPositiveInt(searchParams.get("maxChunksPerItem"));
  if (mc !== undefined) out.maxChunksPerItem = clampInt(mc, 1, 12);
  const rmi = parseOptPositiveInt(searchParams.get("retrievalMaxItems"));
  if (rmi !== undefined) out.maxItems = clampInt(rmi, 8, 80);
  return out;
}
