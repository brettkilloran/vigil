...
function hashString(input: string): number {
  ...
  for (let i = 0; i < input.length; i += 1) {
    // biome-ignore lint/suspicious/noBitwiseOperators: djb2-style hash mixing (hash * 32 - hash)
    hash = (hash << 5) - hash + input.charCodeAt(i);
    // biome-ignore lint/suspicious/noBitwiseOperators: coerce to 32-bit signed int for stable hash accumulation
    hash |= 0;
  }
...
export function EntityGraphPillCanvas({
  ...
  onLayoutChange,
  onNodePin,
  cameraResetKey: _cameraResetKey,
}: {
  nodes: GraphNode[];
  edges: GraphEdge[];
  layout: LayoutMap;
  ...
  useEffect(() => {
    ...
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: rAF tick integrates spring layout: per-node target seek, link-spring forces, drag overrides, and velocity damping each frame
