...
} from "@/src/components/foundation/architectural-button";
import {
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: color strip toggles between label/icon appearance and folder/thread context, each with its own catalog popover, swatch ring, and tooltip wiring
export function ArchitecturalFolderColorStrip({
  ...
  const spoolMenuPortal =
    menuOpen && isSpool && typeof document !== "undefined"
      ? createPortal(
          // biome-ignore lint/a11y/useAriaPropsSupportedByRole: aria-label and role are both gated by threadCatalog; aria-label only appears with the listbox role
          <div
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: dock renders mode-dependent toolbars (canvas/document) with inline-format, create-node, undo/redo, color strip, and pin variants
