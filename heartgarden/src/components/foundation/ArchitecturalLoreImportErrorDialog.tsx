...
export function ArchitecturalLoreImportErrorDialog({ failure, onClose, onRetry }: Props) {
  const [copyHint, setCopyHint] = useState<"idle" | "copied" | "failed">("idle");
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const panelId = useId();
  const panelRef = useRef<HTMLDivElement>(null);
  const canRetry = Boolean(onRetry && failure && RETRYABLE_STAGES.has(failure.stage));

  const report = useMemo(
    () => (failure ? formatLoreImportFailureReport(failure) : ""),
    [failure],
  );
  const statusLine = useMemo(() => (failure ? loreImportSummaryLine(failure) : ""), [failure]);
  ...
  return createPortal(
    <div className={styles.loreImportErrorBackdrop} aria-hidden={false}>
      <div
        ref={panelRef}
        id={panelId}
      ...
      >
        <div id={`${panelId}-title`} className={styles.syncPopoverTitle}>
          Import failed
        </div>
        <div className={styles.syncPopoverStatusLineWrap}>
          <span
            className={`${styles.syncPopoverSeverity} ${
              severity === "warn" ? styles.syncPopoverSeverityWarn : styles.syncPopoverSeverityError
            }`}
          >
        ...
        <p className={styles.syncPopoverAction}>
          {failure.recommendedAction ?? "Copy diagnostics and share it so we can isolate the failing stage."}
        </p>
        <div className={styles.syncPopoverContext}>
          <div className={styles.syncPopoverContextRow}>
            ...
            <span className={styles.syncPopoverContextVal}>{failure.attemptId}</span>
          </div>
          <div className={styles.syncPopoverContextRow}>
            <span className={styles.syncPopoverContextKey}>Stage</span>
            <span className={styles.syncPopoverContextVal}>{failure.stage}</span>
          </div>
          <div className={styles.syncPopoverContextRow}>
            <span className={styles.syncPopoverContextKey}>Operation</span>
            <span className={styles.syncPopoverContextVal}>{failure.operation}</span>
          </div>
          {failure.jobId ? (
            <div className={styles.syncPopoverContextRow}>
              <span className={styles.syncPopoverContextKey}>Job</span>
              <span className={styles.syncPopoverContextVal}>{failure.jobId}</span>
            </div>
          ) : null}
          {failure.phase ? (
            <div className={styles.syncPopoverContextRow}>
              <span className={styles.syncPopoverContextKey}>Phase</span>
              <span className={styles.syncPopoverContextVal}>{failure.phase}</span>
            </div>
          ) : null}
          {typeof failure.httpStatus === "number" ? (
            <div className={styles.syncPopoverContextRow}>
              <span className={styles.syncPopoverContextKey}>HTTP</span>
              <span className={styles.syncPopoverContextVal}>{failure.httpStatus}</span>
            </div>
          ) : null}
        </div>
