...
import { BufferedTextInput } from "@/src/components/editing/buffered-text-input";
...
} from "@/src/components/foundation/architectural-folder-card";
import {
...
import { ArchitecturalLoreFactionArchiveCanvasNode } from "@/src/components/foundation/architectural-lore-faction-archive-canvas-node";
import { ArchitecturalLoreImportErrorDialog } from "@/src/components/foundation/architectural-lore-import-error-dialog";
import {
  ArchitecturalLoreImportUploadPopover,
  type LoreImportScopeMode,
  type LoreImportUploadMode,
} from "@/src/components/foundation/architectural-lore-import-upload-popover";
...
} from "@/src/components/foundation/architectural-lore-review-panel";
import {
...
} from "@/src/components/foundation/architectural-status-bar";
import { ArchitecturalToolRail } from "@/src/components/foundation/architectural-tool-rail";
import {
...
import { CanvasMinimap } from "@/src/components/foundation/canvas-minimap";
import { CanvasViewportToast } from "@/src/components/foundation/canvas-viewport-toast";
...
import { AltGraphCard } from "@/src/components/product-ui/canvas/alt-graph-card";
import { GraphPanel } from "@/src/components/product-ui/canvas/graph-panel";
...
import {
  ...
  type PaletteItem,
  type PaletteSpace,
} from "@/src/components/ui/command-palette";
import {
  ContextMenu,
...
import { HeartgardenMediaPlaceholderImg } from "@/src/components/ui/heartgarden-media-placeholder-img";
import { LinkGraphOverlay } from "@/src/components/ui/link-graph-overlay";
...
import { VigilAppBootScreen } from "./vigil-app-boot-screen";
import { VigilAppChromeAudioMuteButton } from "./vigil-app-chrome-audio-mute-button";

const VigilFlowRevealOverlay = dynamic(
  () =>
    import(
      "@/src/components/transition-experiment/vigil-flow-reveal-overlay"
    ).then((mod) => ({
...
interface LoreSmartImportReviewState {
  fileName?: string;
  plan: LoreImportPlan;
  sourceText: string;
  sourceTitle?: string;
...
interface LoreImportSelectionState {
  contextText: string;
  mode: LoreImportUploadMode;
  scope: LoreImportScopeMode;
}

interface LoreImportPreparedSource {
  fileName: string;
...
interface LoreImportJobProgress {
  message?: string;
...
interface LoreImportTargetSpaceOption {
  path: string;
  spaceId: string;
  title: string;
}
...
const ALLOWED_LORE_IMPORT_JOB_EVENT_KINDS = new Set<LoreImportJobEvent["kind"]>(
  ["phase_start", "phase_end", "llm_call", "vault_search", "warning", "note"]
);
...
function nonNegativeIntOrUndefined(value: unknown): number | undefined {
  const n = coerceOptionalProgressInt(value);
...
function normalizeLoreImportJobEvents(raw: unknown): LoreImportJobEvent[] {
  ...
  return raw
    .filter((entry): entry is Record<string, unknown> => {
      ...
      ).trim() as LoreImportJobEvent["kind"];
      return ALLOWED_LORE_IMPORT_JOB_EVENT_KINDS.has(kind);
    })
    .map((row) => ({
      durationMs: nonNegativeIntOrUndefined(row.durationMs),
      kind: String(row.kind ?? "").trim() as LoreImportJobEvent["kind"],
      model: trimmedOrUndefined(row.model),
      phase: trimmedOrUndefined(row.phase),
      ref: trimmedOrUndefined(row.ref),
      responseSnippet: trimmedOrUndefined(row.responseSnippet),
      stopReason: trimmedOrUndefined(row.stopReason),
      text: trimmedOrUndefined(row.text),
      tokensIn: nonNegativeIntOrUndefined(row.tokensIn),
      tokensOut: nonNegativeIntOrUndefined(row.tokensOut),
      ts: trimmedOrUndefined(row.ts),
    }));
}
...
const QUEUED_PREFIX_RE = /^queued\b/i;
const SESSION_PREFIX_RE = /^Session:\s*/i;
const SOURCE_PREFIX_RE = /^Source:\s*/i;

function loreImportSuggestedTitle(fileName: string): string {
  const trimmed = fileName.trim();
  const base = trimmed.replace(FILENAME_EXTENSION_RE, "").trim();
  return base || "Import";
}
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: in-browser PDF text extraction loop with per-page abort checks, page-load and getTextContent failure tallying, and char-cap truncation guard
...
interface LoreImportOtherFollowUp {
...
function collectDeletionClosure(
  ...
  const markEntity = (entityId: string) => {
    ...
    while (stack.length > 0) {
      ...
      for (const id of space.entityIds) {
        markEntity(id);
      }
      for (const candidate of Object.values(graph.spaces)) {
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: connection-pin resolution covers item/folder/space targets and slot offsets, with stack-fan stage preference, live DOM anchor lookups, and rotation-aware fallback math
...
function applyUnstackStackInSpace(
  ...
  const anchor = members.at(-1)?.slots[spaceId] ??
...
function getStackSelectionState(
  ...
  for (const sid of whollySelectedStackIds) {
    const members = stackGroups.get(sid);
    if (members) {
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: checklist markup normalizer walks legacy + TipTap HTML shapes (li[data-task] / .taskItem / inline checkbox / nested fragment) into the canonical class+attr form
function normalizeChecklistMarkup(
  ...
  for (const ul of wrap.querySelectorAll("ul[data-arch-checklist='true']")) {
  ...
  for (const taskItemEl of wrap.querySelectorAll(`.${classes.taskItem}`)) {
    ...
    if (taskText) {
      ...
      for (const node of Array.from(taskItem.childNodes)) {
 ...
 */
interface WorkspaceBootstrapErrorSummary {
    ...
    >
      ...
      >
        ...
        <div className={styles.neonWorkspaceUnavailableCopySection}>
          ...
          <pre className={styles.neonWorkspaceUnavailablePre}>
            {copyPayload}
          </pre>
        </div>
...
interface HeartgardenBootStatusJson {
  gateEnabled?: boolean;
...
export interface HeartgardenBootApiState {
  gateEnabled: boolean;
  loaded: boolean;
  playerLayerMisconfigured: boolean;
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: top-level canvas shell holds graph state, navigation, focus/gallery/stack modal flow, lore import, presence, and Neon sync wiring; subsystems stay co-located so refs and memo deps line up
export function ArchitecturalCanvasApp({
  ...
  const [loreSmartPlanningDetailsOpen, setLoreSmartPlanningDetailsOpen] =
    useState(false);
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: memoized lore-smart-planning UI; conditional rendering across queued/parsing/findings/failed phases with subphase, pipeline percent, and queue-failure hint extraction
  const loreSmartPlanningUi = useMemo(() => {
    ...
    const redundantQueuedCopy =
      phase === "queued" && QUEUED_PREFIX_RE.test(rawDetail);
    const detail =
      redundantQueuedCopy ||
      rawDetail.length === 0 ||
      QUEUED_PREFIX_RE.test(rawDetail)
        ? null
        : rawDetail;
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: memoized lore-smart-question UI model; derives required vs optional clarification counts, stable order, focus question, and progress bar percent across answered/unanswered states
  ...
  const patchItemWithVersion = useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: PATCH with version reconciles in-flight create promises and 409 retry; on remote 410-gone prunes graph, recents, selection, and any open focus/gallery surface tied to the item
  ...
  const schedulePersistContentBody = useCallback(
    (entityId: string) => {
      ...
      const t = setTimeout(() => {
        ...
        patchItemWithVersion(entityId, patch).catch(() => {
          /* errors logged in PATCH layer */
        });
      }, 450);
  ...
  const persistNeonItemsLayout = useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: persists Neon items + spaceId after parent/folder reparent; reconciles measured DOM placement vs entity dims and skips uuid-likes that are not yet on the server
    (ids: string[], graphSnapshot?: CanvasGraph) => {
      if (!persistNeonRef.current) {
        return;
      }
  ...
  const syncNeonAfterHistoryTransition = useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: undo/redo Neon reconcile diffs entities, connections, and spaces between graph snapshots and emits add/delete/reparent calls, including space-deletion-root filtering
  ...
  const undo = useCallback(() => {
    ...
    const restore = undoPastRef.current.at(-1)!;
  ...
  const redo = useCallback(() => {
    ...
    const restore = undoFutureRef.current.at(-1)!;
  ...
  const applyConnectionKind = useCallback(
    (nextKind: ConnectionKind) => {
      ...
      for (const id of between) {
        recolorConnection(id, nextColor);
      }
    },
    [recolorConnection]
  );
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: one-shot canonical color/linkType migration gated by bootstrap; computes pair per kind, skips already-canonical, and persists via PATCH while skipping undo
  useEffect(() => {
   ...
   */
  const _activeSpaceEntityIdsFingerprint = `${activeSpaceId}\u0001${activeSpaceEntityIdsRaw.join("\u0001")}`;
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      for (const connection of Object.values(graphSnap.connections)) {
        ...
        // biome-ignore lint/style/noNonNullAssertion: rope runtime is constructed with a fixed point count; first/last always exist
        const first = runtime.points[0]!;
        // biome-ignore lint/style/noNonNullAssertion: rope runtime is constructed with a fixed point count; first/last always exist
        const last = runtime.points.at(-1)!;
        first.x = start.x;
        first.y = start.y;
        first.oldX = start.x;
        first.oldY = start.y;
        ...
        const penultimate = runtime.points.at(-2);
        if (penultimate && last) {
          path += ` Q ${penultimate.x} ${penultimate.y}, ${last.x} ${last.y}`;
        }
        nextPaths[connection.id] = path;
      }
      for (const id of Object.keys(runtimeById)) {
        if (!activeIds.has(id)) {
      ...
      if (svgRoot) {
        for (const pathEl of svgRoot.querySelectorAll<SVGPathElement>(
  ...
  const nodeZ = useMemo(() => {
    ...
    for (const [index, entity] of activeSpaceEntities.entries()) {
  ...
  const stackGroups = useMemo(() => {
    const groups = new Map<string, CanvasEntity[]>();
    for (const entity of activeSpaceEntities) {
      if (!entity.stackId) {
        continue;
      }
      const arr = groups.get(entity.stackId) ?? [];
  ...
  const _stackFocusBoundsEffectKey = useMemo(() => {
  ...
  const _stackHoverBoundsEffectKey = useMemo(() => {
    if (collapsedStacks.length === 0) {
      return "empty";
    }
    const stackPart = collapsedStacks
  ...
  useEffect(() => {
    ...
    for (const { stackId, entities } of collapsedForBounds) {
  ...
  useEffect(() => {
    ...
    > = {};
    const domRoot: ParentNode = shellRef.current ?? document;
    for (const { stackId } of collapsedForBounds) {
      const container = domRoot.querySelector<HTMLElement>(
        `[data-stack-container='true'][data-stack-id='${stackId}']`
      );
      if (!container) {
  ...
  const acceptAiReviewForEntity = useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: AI-review accept strips hgAiPending marks from hgDoc JSON or HTML body (whichever the entity uses), then clears entityMeta.aiReview optimistically and persists via patchItemWithVersion
    (entityId: string) => {
      const ent = graphRef.current.entities[entityId];
      if (!ent || ent.kind !== "content") {
        return;
  ...
  const onArchitecturalMediaFile = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = () => {
        ...
        const alt =
          file.name
            .replace(FILENAME_EXTENSION_RE, "")
            .replace(NON_WORD_SEPARATORS_RE, " ")
            .trim() || "Uploaded image";
  ...
  const mergeRemoteGraphEdgesImpl = useCallback(
    ...
    [activeSpaceId, scenario, canvasBootstrapResolved]
  );
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: bootstrap loader: success/failure branching across open vs gated workspaces, cache-snapshot fallback for strict GM, and demo local-canvas shim when Neon is absent
    void (async () => {
  ...
  }, [scenario, ingestLiveBootstrap, applyDemoLocalCanvas]);
  ...
  const _handleLogOutToAuth = useCallback(() => {
    if (scenario !== "default") {
      return;
    }
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: lore-import apply: validates Neon target, packages plan + override targets + accepted merges, awaits /api/lore/import/apply, and routes failure detail per stage/operation
  ...
  const moveEntitiesToSpace = useCallback(
    (
      ...
      // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: cross-space move resolves grid-layout slots, stack regrouping, parent removal, and forced-layout overrides while preserving relative offsets
      const apply = (prev: CanvasGraph) => {
        ...
        for (const id of remainingDestinationIds) {
          const entity = next.entities[id];
  ...
  const enterSpace = useCallback(
    (spaceId: string, opts?: { cameraOverride?: CameraState }) => {
      ...
      if (!canvasEffectsEnabledRef.current) {
        // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: enterSpace effects-off fast path: bootstrap merge, max-z and sync-cursor reconciliation, all guarded by stillCurrent so superseded navigations cannot rewind active-space state
        void (async () => {
      ...
      // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: enterSpace transition with canvas effects on: timed dim/flow overlay phases interleaved with bootstrap fetch, stillCurrent guard, and buffered sync-cursor commit
      void (async () => {
        const now = () =>
  ...
  const collabPeerChips = useMemo((): CollabPeerPresenceChip[] => {
    ...
    const now = Date.now();
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: builds presence chips: identity-on vs anonymous fallback (alias/emoji/sigil), staleness windowing, follow-handler wiring, and tooltip composition
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: vault-review draft picks source from focus mode vs single-selection vs current space, excluding lore canvas node variants and stripping legacy HTML for plain-text body
  ...
  const appendVaultReviewTags = useCallback(
    async (tags: string[]): Promise<boolean> => {
      ...
      return await patchItemWithVersion(draft.excludeItemId, {
      ...
      queueGraphCommit(
        ...
        () => {
          ...
          setGraph((p) => {
            ...
            const next = shallowCloneGraph(p);
            if (scheme == null) {
              const { folderColorScheme: _drop, ...updated } = ent;
          ...
          queueMicrotask(() => {
            void (async () => {
              ...
              const withTargetScheme = (folder: CanvasFolderEntity) => {
                if (scheme == null) {
                  const { folderColorScheme: _drop, ...updated } = folder;
                  return updated;
                }
  ...
  const createNewNode = useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: per-NodeTheme creation forks (folder vs note vs lore variants) with optimistic Neon insert + rollback paths and strict-GM gating
    (type: NodeTheme, loreVariant?: LoreCardVariant) => {
      ...
      if (persistNeonRef.current && isUuidLike(activeSpaceId)) {
        ...
        if (type === "folder") {
          ...
          // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: optimistic folder creation: reverts entity, child space, item, and selection on space-or-item create failure across two awaited Neon calls
  ...
  const executeLoreImportWithParsed = useCallback(
    async (args: {
      ...
      // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: granularity dispatch (one_note vs smart-plan) plus Neon prerequisite checks and planning-abort error mapping for parsed lore-import payloads
    }) => {
      const {
        parsedText,
        parsedFileName,
  ...
  const onLoreImportFileChange = useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: lore-import file flow: PDF parse fallback chain (preflight oversize -> server -> 413 local fallback), text vs PDF dispatch, then plan/smart-review handoff
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (
          (!parsed?.ok || typeof parsed.text !== "string") &&
          parseRes?.status === 413 &&
          isPdfFile
        ) {
  ...
  const runPaletteAction = useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: command-palette dispatch maps ~20+ action ids to creators, panels, and modals with restricted-layer gating per action
    (actionId: string) => {
      if (
        isRestrictedLayer &&
        [
  ...
  const updateDropTargets = useCallback(
    (
      ...
      // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: drop-target resolution covers folder-card hits, stack-merge hits with hull bounds, and connection-pin overlap, with shared-stack vs single-node DOM rects
    ) => {
      const domRoot: ParentNode = shellRef.current ?? document;
      const draggedGroup =
      ...
      if (!(stackModalRef.current || nextFolderId) && canDragGroupStack) {
        for (const targetEl of Array.from(
  ...
  const stackEntitiesOntoTarget = useCallback(
    (draggedEntityIds: string[], targetEntityId: string) => {
      ...
      setGraph((prev) => {
        ...
        for (const oldStackId of normalizedOldStackIds) {
  ...
  useEffect(() => {
    ...
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: lasso completion: click-vs-drag detection, center-point containment hit-test against allowed space ids, and Shift/Cmd additive selection branching
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: stable pan helper; full handleDrop/updateDropTargets/centerCoords closure intentionally omitted to avoid re-binding window pointermove every pan frame.
    flushPanPending,
    // biome-ignore lint/correctness/useExhaustiveDependencies: stable pan helper; full handleDrop/updateDropTargets/centerCoords closure intentionally omitted to avoid re-binding window pointermove every pan frame.
    cancelPanRaf,
    // biome-ignore lint/correctness/useExhaustiveDependencies: stable pan helper; full handleDrop/updateDropTargets/centerCoords closure intentionally omitted to avoid re-binding window pointermove every pan frame.
    scheduleUpdateDropTargets,
    // biome-ignore lint/correctness/useExhaustiveDependencies: stable pan helper; full handleDrop/updateDropTargets/centerCoords closure intentionally omitted to avoid re-binding window pointermove every pan frame.
    schedulePanTranslate,
  ]);
  ...
  useEffect(() => {
    ...
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: stack-modal mouseUp commits eject vs reorder; reconciles hull snapshots, drag-start order, pointer movement threshold, and persists Neon reparent
    const onMouseUp = (event: MouseEvent) => {
      ...
      if (outsideWithMargin && pointerMoved) {
        ...
        if (extracted) {
          ...
          const anchorSource = sortedRemaining.at(-1) ?? sortedRemaining[0];
  ...
  useEffect(() => {
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: global mousedown router dispatches across sync popover, task checkbox toggle (focus body vs node body), connection draw/cut, and lasso start
    ...
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: canvas double-click router: opens focus on content/lore variants, enters folder spaces, expands stacks via fan modal, while exiting early on draw/cut tool surfaces
  ...
  const deleteEntitySelection = useCallback(
    (entityIds: string[]) => {
      ...
      setGraph((prev) => {
        ...
        for (const spaceId of closure.spaceIds) {
          if (spaceId !== next.rootSpaceId) {
            delete next.spaces[spaceId];
          }
        }

        for (const cid of Object.keys(next.connections)) {
      ...
      if (persistNeonRef.current) {
        void (async () => {
          ...
          const reparentResults = await Promise.all(
            reparentOps
              .filter((reparent) => isUuidLike(reparent.spaceId))
              .map((reparent) =>
                apiPatchSpaceParent(reparent.spaceId, reparent.newParentId)
              )
          );
          if (
  ...
  const stackModalHull = useMemo(() => {
    ...
    let maxX = Number.NEGATIVE_INFINITY;
    let maxY = Number.NEGATIVE_INFINITY;
    for (const entity of stackModalVisibleEntities) {
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {/* biome-ignore lint/complexity/noExcessiveCognitiveComplexity: standalone entity renderer projects each kind (note/folder/sticky/image/lore) with selection, drag, drop preview, and pin overlays */}
                ...
                {visibleCollapsedStacks.map(({ stackId, entities, top }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* biome-ignore lint/complexity/noExcessiveCognitiveComplexity: stack layer renderer projects each entity to its node card variant (note/folder/sticky/image/lore) with hover/selection/connection wiring */}
                      {entities.map((entity, index) => {
                        const isTopStackLayer = index === entities.length - 1;
                        return (
                          // biome-ignore lint/a11y/useKeyWithClickEvents: stack layer is the canvas chrome wrapper; the inner node card child has the canonical interactive affordance (button/input/contentEditable) with proper keyboard handling.
                          <div
                            ...
                            onClick={
                              isTopStackLayer
                                ? // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: top-stack layer click promotes selection through expand/preview/folder/pin variants and respects connectionMode
                                  (event) => {
                                    if (connectionMode !== "move") {
                                      return;
                                    }
                          ...
                          <div className={styles.smartImportPlanningTimeline}>
                            {loreSmartPlanningEventGroups.map((group) => (
                              ...
                              >
                                ...
                                >
                                  {/* biome-ignore lint/complexity/noExcessiveCognitiveComplexity: failed-phase event row branches by event.kind (phase_start/phase_done/error/log) with per-kind formatting */}
                                  {group.events.map((event, idx) => (
                                    <li
                                      className={
                                        styles.smartImportPlanningTimelineItem
                                      }
                                      // biome-ignore lint/suspicious/noArrayIndexKey: import job timeline events have no stable id; group.phase + idx is the most stable composite available
                          ...
                          <div className={styles.smartImportPlanningTimeline}>
                            {loreSmartPlanningEventGroups.map((group) => (
                              ...
                              >
                                ...
                                >
                                  {/* biome-ignore lint/complexity/noExcessiveCognitiveComplexity: per-event timeline row branches by event.kind (phase_start/phase_done/error/log) with per-kind formatting and detail strings */}
                                  {group.events.map((event, idx) => (
                                    <li
                                      className={
                                        styles.smartImportPlanningTimelineItem
                      ...
                      {loreSmartReview.plan.clarifications.length === 0 ? (
                        ...
                        <>
                          ...
                          {loreSmartOtherFollowUp ? (
                            ...
                            // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: IIFE renders the focused clarification card with single/multi/other-text answer branches and per-option select state
                            (() => {
        ...
        {stackModal ? (
          // biome-ignore lint/a11y/useKeyWithClickEvents: stack modal scrim is the dismiss-on-outside-click backdrop; keyboard close is handled by the global Esc handler in the canvas shell.
            ...
            <div className={styles.focusSheet}>
              ...
              >
                ...
                <div className={styles.mediaGalleryAssetStage}>
                  {galleryRaster.src ? (
                    // biome-ignore lint/performance/noImgElement: dynamic user/R2 URLs; not suitable for next/image without broad remotePatterns. TODO: configure next.config remotePatterns for the R2 host(s) and migrate to next/image for proper width/quality optimization
                    // biome-ignore lint/correctness/useImageSize: dynamic dimensions populated from onLoad measurement; intrinsic size not known at render
                    <img
