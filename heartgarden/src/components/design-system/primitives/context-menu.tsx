      ...
      {items.map((it, i) =>
        it.type === "heading" ? (
          <div
            ...
            // biome-ignore lint/suspicious/noArrayIndexKey: ContextMenuItem has no stable id; labels may repeat across menus, so composite `${i}-${label}` is the most stable key available
            key={`h-${i}-${it.label}`}
            role="presentation"
          >
            {it.label}
