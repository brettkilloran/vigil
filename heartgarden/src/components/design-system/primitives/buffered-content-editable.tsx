...
import { SlashCommandAssistPopover } from "@/src/components/editing/slash-command-assist-popover";
import {
  ...
  return (
    <>
      <div
        ...
        // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: onKeyDown dispatches across slash assist nav, Enter, Backspace, Delete, list/task collapses, and shortcut keys for the editable element
        onKeyDown={(event) => {
                ...
                if (taskItem && root.contains(taskItem)) {
                  ...
                  if (
                    ...
                    if (taskTextIsVisuallyEmpty(taskText)) {
                      ...
                      taskItem.remove();
                      onDraftChange(readElementValue());
                      // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: rAF callback restores caret across prev/next checklist neighbors after Backspace removes the empty task
                      requestAnimationFrame(() => {
                        const r = ref.current;
                        if (!r?.isConnected) {
                          return;
            ...
            if (root && sel?.isCollapsed && sel.rangeCount > 0) {
              ...
              if (
                ...
                if (taskItem && root.contains(taskItem)) {
                  ...
                  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: rAF callback rehomes caret to prev/next task item or paragraph fallback after task removal
                  requestAnimationFrame(() => {
                    const r = ref.current;
                    if (!r?.isConnected) {
                      return;
              ...
              if (
                ...
                if (list && root.contains(list)) {
                  ...
                  onDraftChange(readElementValue());
                  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: rAF callback restores caret to prevLi/nextLi/listPrev/listNext after a list-item removal collapses the parent list
                  requestAnimationFrame(() => {
                    const r = ref.current;
                    if (!r?.isConnected) {
                      return;
              ...
              if (
                ...
                block.remove();
                onDraftChange(readElementValue());
                // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: rAF callback restores caret across prev/next sibling permutations after removing a checklist block
                requestAnimationFrame(() => {
                  const r = ref.current;
                  if (!r?.isConnected) {
                    return;
