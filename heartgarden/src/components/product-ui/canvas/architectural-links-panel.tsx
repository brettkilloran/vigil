...
import { CanvasDebugInspectorShell } from "@/src/components/ui/canvas-debug-inspector-shell";
...
export function ArchitecturalLinksPanel({
  ...
  selectedEntityIds,
  cloudEnabled,
  itemLinksRevision: _itemLinksRevision,
  onFocusEntity,
}: {
  graph: CanvasGraph;
  ...
  const itemIdForCloud =
    entity && isUuidLike(entity.persistedItemId ?? entity.id)
      ? (entity.persistedItemId ?? entity.id)
      : entity && isUuidLike(entity.id)
        ? entity.id
        : null;
        ...
        {canvasThreads.rows.length === 0 ? (
          ...
          <ul className={debugInspectorStyles.debugInspectorList}>
            {canvasThreads.rows.map((r, i) => (
              // biome-ignore lint/suspicious/noArrayIndexKey: canvas thread rows may share peerId across roles; index disambiguates the duplicates
              <li key={`${r.peerId}-${i}`}>
                <Button
