...
function invalidateLinkRevisionSpaces(
...
function normalizeItemLinkMeta(
...
export async function POST(req: Request) {
  ...
  const validated = await validateLinkTargetsInBrane(db, sourceItemId, [
    targetItemId,
  ]);
  if (!validated.ok) {
    return Response.json(
  ...
  const mirrorCheck = validateStructuredMirrorItemLink(
    metaForRow,
    srcRow,
    tgtRow
  );
    ...
    const [existing] = await db
      ...
      .where(
        and(
          ...
          eq(itemLinks.targetItemId, targetItemId),
          pinClause
        )
      )
      .limit(1);
...
export async function PATCH(req: Request) {
  ...
  if (linkType !== undefined) {
    updates.linkType = normalizeLinkTypeAlias(linkType);
  }
  if (meta !== undefined) {
    if (meta === null) {
      updates.meta = null;
  ...
  if (updates.linkType !== undefined && updates.meta === undefined) {
    const prev =
      ...
      !Array.isArray(existing.meta)
        ...
        : {};
    updates.meta = normalizeItemLinkMeta(
      withLinkSemanticMeta(prev, updates.linkType)
    );
  }
  ...
  let updated: typeof itemLinks.$inferSelect | undefined;
