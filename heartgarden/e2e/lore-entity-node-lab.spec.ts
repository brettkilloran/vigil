...
test.describe("/dev/lore-entity-nodes", () => {
  ...
  test("shows location v2–v7 seeded previews", async ({ page }) => {
    ...
    await expect(
      page.getByText("V4 · Ordo · coordinate slab (mono)")
    ).toBeVisible();
    await expect(
      page.getByText("V5 · Ordo · coordinate slab (mono, field-mapped)")
    ).toBeVisible();
    await expect(
      page.getByText("V6 · Ordo · coordinate slab (mono, lean)")
    ).toBeVisible();
    await expect(
      page.getByText("V7 · Ordo · coordinate slab (mono, hgDoc notes)")
    ).toBeVisible();
    await expect(page.getByTestId("loc-survey-v3")).toBeVisible();
    await expect(
      page.getByTestId("loc-lab-ordo-coordinate-mono")
    ).toBeVisible();
    await expect(
      page.getByTestId("loc-lab-ordo-coordinate-mono-v5")
    ).toBeVisible();
    await expect(
      page.getByTestId("loc-lab-ordo-coordinate-mono-v6")
    ).toBeVisible();
    const v7Slab = page.getByTestId("loc-lab-ordo-coordinate-mono-v7");
    await expect(v7Slab).toBeVisible();
    await expect(
      v7Slab.locator('[data-hg-lore-location-staple="v7"]')
    ).toBeVisible();
  });

  test("shows faction lab plates through XX including shelf, sleeve, and protocol specimens", async ({
    page,
  }) => {
    await page.goto("/dev/lore-entity-nodes");
    await expect(
    ...
    await expect(
      page.getByText("V · Shelf · sleeve perforation")
    ).toBeVisible();
    await expect(
      page.getByText("VI · Protocol · ORDO LUNARIS · compact", { exact: true })
    ).toBeVisible();
    await expect(page.getByText("VII · Protocol · ORDO LUNARIS")).toBeVisible();
    await expect(
      page.getByText("VIII · Classified · Silent Synod")
    ).toBeVisible();
    await expect(page.getByText("IX · Archive · 091 internal")).toBeVisible();
    await expect(
      page.getByText("X · Protocol · Lattice induction")
    ).toBeVisible();
    await expect(page.getByText("XI · Terminal · AEON Conclave")).toBeVisible();
    await expect(
      page.getByText("XII · Protocol · AEON classified")
    ).toBeVisible();
    ...
    await expect(
      page.getByTestId("fac-lab-protocol-ordo-compact")
    ).toBeVisible();
    ...
    await expect(
      page.getByTestId("fac-lab-protocol-archive-091")
    ).toBeVisible();
    await expect(page.getByTestId("fac-lab-protocol-lattice")).toBeVisible();
    await expect(
      page.getByTestId("fac-lab-protocol-aeon-conclave")
    ).toBeVisible();
    await expect(
      page.getByTestId("fac-lab-protocol-aeon-protocol")
    ).toBeVisible();
    await expect(
      page.getByText("XVIII · Bureau · Clandestine circulation brief")
    ).toBeVisible();
    await expect(
      page.getByTestId("fac-lab-protocol-clandestine-brief-xviii")
    ).toBeVisible();
    await expect(
      page.getByText("XIX · Ocular · Mandate dossier (light)")
    ).toBeVisible();
    await expect(
      page.getByTestId("fac-lab-protocol-ocular-mandate-light-xix")
    ).toBeVisible();
    await expect(
      page.getByText("XX · Archive · 091 internal (readable)")
    ).toBeVisible();
    await expect(
      page.getByTestId("fac-lab-protocol-archive-091-readable-xx")
    ).toBeVisible();
  });
});
