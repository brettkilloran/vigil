...
`HEARTGARDEN_MCP_SERVICE_KEY=… pnpm run mcp:smoke`
...
| GET | `/api/spaces/[spaceId]/items` | Items for space. **Paginated** (`REVIEW_2026-04-25_1835` M2): default `limit=500`, max `limit=1000`, optional `offset=<n>` for paging, or `limit=all` to fetch everything (export tooling). Response includes `total`, `offset`, `limit`, and `nextOffset` (null when finished). |
...
| GET | `/api/spaces/[spaceId]/graph` | Graph JSON export for one space. Returns `{ ok, nodes, edges, total_nodes, itemLinksRevision, graphRevision, layoutVersion, layoutCacheHit }` and, on cache hit for full-graph requests (no `limit`), `layoutPositions` + `layoutCacheSavedAt`. Supports paging via `limit` (max 2000) + `offset`; paged responses include `limit`, `offset`, and `note` and intentionally omit layout cache attachment. |
| PUT | `/api/spaces/[spaceId]/graph-layout-cache` | Upsert persisted graph node positions for warm-start. Body: `{ graphRevision, layoutVersion?, nodeCount?, positions }` where `positions` is `Record<nodeId,{x,y,z?}>`. Enforces space access and payload guardrails (400 invalid body, 413 oversize). |
...
| POST | `/api/lore/import/jobs/cleanup` | **GM-only TTL prune** of `lore_import_jobs`. Body `{ redactAfterDays?: number, purgeAfterDays?: number, dryRun?: boolean }`. Defaults: redact `source_text`/`plan`/`progress_*` on terminal (`applied`/`cancelled`/`failed`) jobs older than 7 days; delete terminal jobs older than 30 days. Caps at 5,000 rows per call so it can be looped from cron. (`REVIEW_2026-04-25_1835` H11.) |
...
| GET | `/api/graph/brane?braneId=<uuid>` | Brane graph endpoint for the canvas **Graph panel**. Defaults to a neighborhood fetch (seed item + `maxDepth`) and supports `limit` caps with truncation metadata; `mode=full` is available for small branes. Returns nodes (`id`, `title`, `itemType`, `entityType`, `spaceId`) plus **explicit edges** (`item_links`) and **implicit edges** (`entity_mentions`), and emits an ETag so unchanged graphs can return `304`. **GM-only**. |
...
| Method | Path | Purpose |
|--------|------|---------|
| GET | `/api/v1/items?space_id=` | List items as v1 JSON. **Paginated** (`REVIEW_2026-04-25_1835` M2): default `limit=500`, max `limit=1000`, optional `offset=<n>`, or `limit=all` to opt out (cannot combine with `offset`). Response includes `total`, `limit`, `offset`, and `next_offset`. |
...
| `HEARTGARDEN_REALTIME_URL` / `HEARTGARDEN_REALTIME_REDIS_URL` / `HEARTGARDEN_REALTIME_SECRET` | Optional multiplayer realtime (**`POST /api/realtime/room-token`**, **`pnpm run realtime`**) — see **`docs/VERCEL_ENV_VARS.md`** |
