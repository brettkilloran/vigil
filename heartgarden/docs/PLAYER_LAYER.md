...
- **Optional realtime:** With **`HEARTGARDEN_REALTIME_*`** env + a separate **`npm run realtime`** process, clients subscribe over **`wss://`** ( **`POST /api/realtime/room-token`** ) so **Redis** invalidations wake tabs immediately; **Neon** and **`GET …/changes`** remain authoritative. **`itemLinksRevision`** + **`GET …/link-revision`** still apply when the socket is down. Deploy and env: [`DEPLOY_VERCEL.md`](./DEPLOY_VERCEL.md) §5.5, [`VERCEL_ENV_VARS.md`](./VERCEL_ENV_VARS.md); HTTP: [`API.md`](./API.md) (Realtime), [`CODEMAP.md`](./CODEMAP.md) (Optional realtime).
