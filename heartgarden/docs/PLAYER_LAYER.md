...
1. From **`heartgarden/`**, preview impact:  
   `HEARTGARDEN_SEVER_WORLDS_DRY=1 pnpm run db:sever-worlds`
2. Destructive run (deletes all **items** and **folder spaces** under player roots, removes **cross-world** links; keeps implicit / env root **space rows** empty):  
   `HEARTGARDEN_SEVER_WORLDS_CONFIRM=1 pnpm run db:sever-worlds`
...
- **Optional realtime:** With **`HEARTGARDEN_REALTIME_*`** env + a separate **`pnpm run realtime`** process, clients subscribe over **`wss://`** ( **`POST /api/realtime/room-token`** ) so **Redis** invalidations wake tabs immediately; **Neon** and **`GET …/changes`** remain authoritative. **`itemLinksRevision`** + **`GET …/link-revision`** still apply when the socket is down. Deploy and env: [`DEPLOY_VERCEL.md`](./DEPLOY_VERCEL.md) §5.5, [`VERCEL_ENV_VARS.md`](./VERCEL_ENV_VARS.md); HTTP: [`API.md`](./API.md) (Realtime), [`CODEMAP.md`](./CODEMAP.md) (Optional realtime).
