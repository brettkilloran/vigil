...
| **Lore import + review flags** | Import opens an upfront mode popover: **One note** (skip planner, create one card), **Many loose** (default; entity extraction on current canvas without folders), **Many in folders** (existing structured mode). Review adds recovery actions: **Collapse to one note** and **Flatten to Nearby** before apply. Smart `/jobs` → `/apply` wraps imported bodies with pending spans and sets **`aiReview: pending`**; merge flows extend structured HTML without flattening prior body. Legacy `/extract` + `/commit` routes are deprecated and return HTTP 410 unless **`HEARTGARDEN_IMPORT_LEGACY_ENABLED=1`**. | [`API.md`](./API.md) (lore import) | `ArchitecturalCanvasApp.tsx`, `ArchitecturalLoreImportUploadPopover.tsx`, `lore-import-plan-reshuffle.ts`, `lore-import-apply.ts`, `app/api/lore/import/*` |
