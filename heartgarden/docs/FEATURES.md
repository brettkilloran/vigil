...
| **Alt-hover discovery (term graph)** | Holding **Alt** and hovering body text highlights the word under the cursor and opens a small popover (**Alt graph card**) showing items that mention that term and search hits across the workspace. Works across folders inside a brane (cross-space mention edges). Click while Alt-held to open the full **Graph panel**, which now uses the canonical Three.js renderer path. | [`API.md`](./API.md) (graph + mentions routes), [`PLAYER_LAYER.md`](./PLAYER_LAYER.md) | `ArchitecturalCanvasApp.tsx` (Alt-hover effect), `AltGraphCard.tsx`, `GraphPanel.tsx`, `EntityGraphThreeCanvas.tsx`, `entity-mentions.ts`, `entity-vocabulary.ts`, `word-under-pointer.ts`, `entity_mentions` / `branes` tables |
| **Dev: entity graph panel UI lab** | `/dev/entity-graph-panel` — production-shaped panel refinement surface (Three renderer + realistic fixtures + panel-width controls) for iterating sidebars/buttons/chrome without changing trigger semantics in the main app. | [`ENTITY_GRAPH_RENDERER_BAKEOFF.md`](./ENTITY_GRAPH_RENDERER_BAKEOFF.md) | `app/dev/entity-graph-panel/*`, `src/components/dev/EntityGraphPanelLab.tsx` |
