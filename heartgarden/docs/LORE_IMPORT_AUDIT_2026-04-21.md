...
| O-17 | Add retention jobs: TTL on `lore_import_jobs` ≥ 7 days with `status='ready'|'failed'`; cascade orphan `import_review_items`. | New cron/after-job. | §5.9, §5.10. |
...
- [x] Goals/purpose reviewed against shipped features.
- [x] Import pipeline (parse → plan → apply → reindex) mapped.
- [x] Sort (folders/spaces), categorize (types/entity_meta), link (item_links / hgArch / prose) mapped.
- [x] Mismatches, gaps, optimizations called out with file-level pointers.
